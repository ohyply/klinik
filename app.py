...
# --- SAYFA YAPILANDIRMASI VE ÖZEL CSS (Boşlukları Azaltma / Ortalama) ---
logo_url = "https://img1.wsimg.com/isteam/ip/024a29d0-042a-4100-bcb5-a12e83d93ed4/c0548627-44f3-4f3a-bba1-8dd4865abcc4.jpg/:/rs=w:256,h:250,cg:true,m/cr=w:256,h:250/qt=q:95"

try:
    response = requests.get(logo_url)
    img = Image.open(BytesIO(response.content))
    st.set_page_config(
        page_title="SepaDent - Akıllı Klinik Asistanı", page_icon=img, layout="wide"
    )
except:
    img = None
    st.set_page_config(
...
# Geniş ekranlardaki aşırı boşlukları önleyen ve tasarımı toparlayan CSS
st.markdown("""
    <style>
    .block-container {
        max-width: 1100px;
...
# --- BAŞLIK VE LOGO YANYANA YERLEŞİMİ ---
...
with col_title:
    st.markdown("## **SepaDent - Klinik Operasyon ve Asistan Paneli**")

st.markdown("Günlük iş yükünü sıfırlayan, tam entegre akıllı klinik yönetim sistemi.")
...
with tab1:
    ...
    for malzeme, miktar in klinik_verisi["stoklar"].items():
        col1, col2, col3, col4, col5 = st.columns([4, 1.5, 0.8, 0.8, 2])
    ...
    if st.button("📲 Tek Tıkla Tedarik Listesi Gönder", key="tedarik_gonder_btn"):
...
with tab3:
    ...
    for lab_idx, lab_is in enumerate(klinik_verisi["laboratuvar"]):
        with st.expander(
            f"Hasta: {lab_is['Hasta']} — İşlem: {lab_is['Islem']} (Durum: {lab_is['Durum']})"
        ):
